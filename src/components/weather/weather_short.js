...
class WeatherShort extends Component {
    ...
    render() {
        ...
        return (
            ...
            <div className="weather_short">

                <div className="overflowWrap">

                    <div className="overflowAdjust">

                        <WeatherTable data={this.props.data} th="stperiodforcurrent_alt" td={["icon", "it"]}/>

                        <AreaChart key="s1" width={1015} height={100} data={this.props.data}
                            ...
                            <Area type="monotone" dataKey="FeelsLike" strokeDasharray="5 5" stroke="#facc40"
                                  fillOpacity={1} fill="url(#colorTc)" unit="&deg;"/>
                        </AreaChart>

                        <AreaChart key="s2" width={1015} height={100} data={this.props.data}
                                   margin={{top: 10, right: 30, left: 0, bottom: 0}}>
                            <XAxis dataKey="0" stroke="#bbb" />
                            <YAxis tick={{fontSize: 12}} stroke="#aaa" />
                            <Tooltip/>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f4f4f4"/>
                            <Area type="monotone" strokeDasharray="5 5" dataKey="Humidity" stroke="#246F9C"
                                  fillOpacity={0.1} fill="#00479C" unit="%"/>
                            <Area type="monotone" dataKey="ChancePrecip" stroke="#00479C" fillOpacity={0.2}
                                  fill="#00479C" unit="%"/>
                        </AreaChart>

                        <AreaChart key="s3" width={1015} height={100} data={this.props.data}
                                   margin={{top: 10, right: 30, left: 0, bottom: 0}}>
                            <XAxis dataKey="0" stroke="#bbb" />
                            <YAxis tick={{fontSize: 12}} stroke="#aaa" />
                            <Tooltip/>
